...
export type KappKit = {
  ...
  article?: string
  /** 導入キットではなく「そのOSSで当社が作ったオンプレミスの製品」。文言を変える。 */
  product?: true
}
...
export const KAPP_KITS: Record<string, KappKit> = {
  ...
  codealmanac: { id: 'b81d730eb535b454', label: 'CodeAlmanac 日本語導入・運用キットを見る', price: '税込5,500円（オンプレミス）', article: 'https://katsushi2441.github.io/vwork/articles/2026-09-03-codealmanac-linux-guide.html' },
  // 2026-09-04 受け皿強化4本（Google流入のあるOSS名→商品ページへの導線）
  vikunja: { id: '767ea5a2f7963f2a', label: 'Vikunja 日本語導入・運用キットを見る', price: '税込5,500円（オンプレミス）', article: 'https://katsushi2441.github.io/vwork/articles/2026-09-03-vikunja-japanese-guide.html' },
  docmost: { id: '7495b31ca104b483', label: 'Docmost 日本語導入・運用キットを見る', price: '税込5,500円（オンプレミス）', article: 'https://katsushi2441.github.io/vwork/articles/2026-09-03-docmost-japanese-guide.html' },
  planka: { id: '1364b33c7d1cde58', label: 'Planka 日本語導入・運用キットを見る', price: '税込5,500円（オンプレミス）', article: 'https://katsushi2441.github.io/vwork/articles/2026-09-04-planka-japanese-guide.html' },
  docspell: { id: '7f9481c10a09b560', label: 'Docspell 日本語導入・運用キットを見る', price: '税込5,500円（オンプレミス）', article: 'https://katsushi2441.github.io/vwork/articles/2026-09-04-docspell-japanese-guide.html' },
  pdfmathtranslate: { id: '8adb859f30cb8cc5', label: 'PDFMathTranslate 日本語導入・運用キットを見る', price: '税込5,500円（オンプレミス）', article: 'https://katsushi2441.github.io/vwork/articles/2026-09-04-pdf-translation-selfhosted.html' },
  // paperless-ngx は「日本語OCRが3か所直さないと効かない」実測が売り。
  // 表記ゆれ（paperless / paperless-ngx）どちらのslugでも同じキットへ寄せる。
  'paperless-ngx': { id: '2c8193abe130c90c', label: 'paperless-ngx 日本語導入・運用キットを見る', price: '税込5,500円（オンプレミス）', article: 'https://katsushi2441.github.io/vwork/articles/2026-09-05-paperless-ngx-japanese-ocr.html' },
  ...
  namazu: { id: '224e141f77bd07a8', label: 'Namazu検索×AIチャットの土台（Kurage Light ChatBot・オンプレミス）を見る', price: '税込55,000円（オンプレミス）', product: true, article: 'https://katsushi2441.github.io/vwork/blog/2026-09-04-namazu-ai-chat-knowledge.html' },
  fess: { id: '224e141f77bd07a8', label: 'Fess検索×AIチャットの土台（Kurage Light ChatBot・オンプレミス）を見る', price: '税込55,000円（オンプレミス）', product: true, article: 'https://katsushi2441.github.io/vwork/articles/2026-09-04-fess-fulltext-search-ai-chat.html' },
  postgis: { id: 'a5ac4b9f1fdb6d19', label: 'PostGISで作った商圏分析システム(オンプレミス)を見る', product: true },
  kshoken: { id: 'a5ac4b9f1fdb6d19', label: '設置手順つきのオンプレミス版を見る', product: true },
  // kepler.gl（GSC 8〜10位・月100表示）は「可視化」で、到達圏と人口集計は無い。商圏分析まで欲しい人の出口を置く。
  'kepler-gl': { id: 'a5ac4b9f1fdb6d19', label: 'kepler.gl に到達圏と人口集計を足した商圏分析（Kurage 商圏分析・オンプレミス）を見る', product: true },
  khazard: { id: '02b945f9c87c9d86', label: 'Kurage 土砂災害ハザードマップ（オンプレミス）を見る', price: '税込55,000円（オンプレミス）', product: true },
  krefuge: { id: '162f155897390072', label: 'Kurage 避難所マップ（オンプレミス）を見る', price: '税込55,000円（オンプレミス）', product: true },
  ktsunami: { id: '86b85a63bc426575', label: 'Kurage 津波浸水想定マップ（オンプレミス）を見る', price: '税込55,000円（オンプレミス）', product: true },
}
...
export function kappKitCards(list: Array<{ slug: string; name: string }>, ref: string): string {
  ...
  return `<div class="grid">${rows.map((p) => {
    ...
    const lead = kit.product
      ? `同じ用途で当社が作った、設置手順つきのオンプレミスの製品。開発を依頼せず自社で動かしたい場合の早道です。`
...
export const KAPP_CATEGORY_PICKS: Record<string, KappCategoryPick> = {
  ...
  lowcode: { id: 'c1864cba4ab726b0', name: 'Kurage 業務アプリ土台（kvgwc）',
    lead: 'kintone・サイボウズの代わりに、社内業務アプリを自社サーバーに持つ土台。月額はかかりません。' },
  ...
  aidev: { id: '48ca584977698dcc', name: 'Kurage AIクローラー計測（ktrackgeo）',
    lead: '自社サイトがAIに読まれているかを測るオンプレミスのツール。GA4では見えないAIクローラーを記録します。' },
  accounting: { id: '15abb025dc2ee4f6', name: 'Kurage 請求書（kbilling）',
    lead: '請求書の発行から集金まで自社サーバーに。月額の会計SaaSを増やさずに済みます。' },
  ...
  automation: { id: '232b3c1d11b6a730', name: 'Kurage サイト更新監視（kcheckit）',
    lead: 'サイト更新・補助金情報の監視を自社サーバーに。決まった巡回を人がやらずに済みます。' },
  dms: { id: '61febea74f9c74b0', name: 'Kurage 領収書発行（kinvoice）',
    lead: '領収書をPDFで発行してメール送信するところまで自社サーバーに。' },
  forum: { id: '4bd9a6f3f99cdc05', name: 'Kurage 掲示板（kbbs）',
    lead: '宣伝・求人を無料掲載できる掲示板を自社で持つオンプレミス版。' },
...
export function kappCategoryPanel(slug: string, category: string, ref: string, name: string): string {
  ...
  return `<h2>開発せずに、出来合いのもので済ませる道もあります</h2>`
    + `<p>${name}のように自分で立てて運用するほかに、当社は${label}の仕組みをオンプレミスのアプリとして出しています。`
    ...
    + `<a class="btn" href="https://kappstore.exbridge.jp/?ref=${ref}">オンプレミスのアプリを全部見る</a></div>`
}
