...
function card(project: Project): string {
  ...
  const search = [project.name, project.summary, categoryLabel(project.category), kindLabel, ...textItems(project.keywords)].join(' ').toLowerCase()
  return `<article class="card" data-category="${attr(project.category)}" data-funnel="${attr(kind)}" data-search="${attr(search)}"><div class="card-top"><span class="pill">${h(categoryLabel(project.category))}</span><span class="license">${h(project.license)}</span></div><p class="kind">${h(kindLabel)}</p><h3><a href="${catalogBase}/${attr(project.slug)}/">${h(project.name)}</a></h3><p>${h(project.summary)}</p><div class="card-actions"><a href="${catalogBase}/${attr(project.slug)}/">詳細を見る</a>${project.lpUrl ? `<a href="${attr(project.lpUrl)}">製品・紹介ページ</a>` : ''}${project.brainUrl ? `<a data-brain-for="${attr(project.slug)}" href="${attr(project.brainUrl)}" target="_blank" rel="noopener">Brain手順書</a>` : ''}</div></article>`
}
...
function indexPage(projects: Project[]): string {
  ...
  const top = [...projects]
    .sort((a, b) => Number(Boolean(b.featured)) - Number(Boolean(a.featured)) || (b.stars || 0) - (a.stars || 0) || a.name.localeCompare(b.name))
    .slice(0, TOP_ON_INDEX)
  const indexFaqs = [
...
function categoryPage(category: string, projects: Project[], all: Project[]): string {
  ...
  const list = [...projects].sort((a, b) => Number(Boolean(b.featured)) - Number(Boolean(a.featured)) || (b.stars || 0) - (a.stars || 0) || a.name.localeCompare(b.name))
  const ossCount = list.filter((item) => funnelOf(item) === 'oss').length
  const protoCount = list.length - ossCount
  const faqItems = [
...
function detailPage(project: Project, projects: Project[]): string {
  ...
  const body = `<main class="wrap"><nav class="crumb" aria-label="パンくず"><a href="${catalogBase}/">OSS一覧</a> / <a href="${catalogBase}/c/${attr(project.category)}/">${h(categoryLabel(project.category))}</a> / ${h(project.name)}</nav><section class="detail-hero"><span class="eyebrow">${h(copy.eyebrow)}</span><h1>${h(project.name)}</h1><p class="lead">${h(project.summary)}</p></section><div class="detail-grid"><div><section class="panel"><h2>${h(project.name)}とは</h2><p>${h(project.description)}</p><p><strong>要点:</strong> ${h(project.summary)}</p></section><section class="panel"><h2>主な用途</h2><ul class="checklist">${textItems(project.useCases).map((item) => `<li>${h(item)}</li>`).join('')}</ul></section><section class="panel vibe-guide"><h2>${h(copy.guideTitle)}</h2><p>${copy.guideBody}</p><a class="btn btn-main" href="${attr(copy.ctaUrl)}">${h(copy.guideCtaLabel)}</a></section><section class="panel faq"><h2>${h(copy.faqHeading)}</h2>${allFaqItems.map((item) => `<details><summary>${h(item.question)}</summary><p>${h(item.answer)}</p></details>`).join('')}</section></div><aside><section class="panel facts"><h2>導入情報</h2><div class="fact"><b>ライセンス</b><span>${h(project.license)}</span></div><div class="fact"><b>日本語対応</b><span>${h(project.japaneseStatus)}</span></div>${project.language ? `<div class="fact"><b>主な言語</b><span>${h(project.language)}</span></div>` : ''}${typeof project.stars === 'number' ? `<div class="fact"><b>GitHubスター</b><span>${project.stars.toLocaleString('en-US')}</span></div>` : ''}${project.sourceLpName ? `<div class="fact"><b>media-mesh掲載元</b><span>${h(project.sourceLpName)}</span></div>` : ''}</section><section class="panel cta"><h2>次の行動</h2><p>${h(copy.asideText)}</p>${links}<a class="btn btn-main" href="${attr(copy.ctaUrl)}">${h(copy.asideCtaLabel)}</a></section></aside></div>${related.length ? `<section class="section"><div class="section-title"><h2>同じカテゴリのOSS</h2></div><div class="related">${related.map(card).join('')}</div></section>` : ''}</main>`
