...
import { kappKitLink as kitLink, kappKitPanel, kappCategoryPanel } from './kapp-kits'
...
function funnelCopy(project: Project) {
  ...
  if (funnelOf(project) === 'prototype') {
    return {
      ...
      pageTitle: project.seoTitle || `${name}でできること・日本語ガイド | Kurage`,
      metaDescription: project.seoDescription || `${project.summary} ライセンスと日本語情報に加え、${name}を使ったバイブコーディングによる自社向けシステム開発を案内します。`,
  ...
  return {
    ...
    pageTitle: project.seoTitle || `${name}の日本語導入・OSSカスタマイズ | Kurage`,
    metaDescription: project.seoDescription || `${project.summary} 日本語対応・ライセンス・導入情報に加え、バイブコーディングによる自社向けOSSカスタマイズとサーバー導入を案内します。`,
...
<footer><div class="wrap"><div class="footer-links"><a href="${catalogBase}/">OSS一覧</a><a href="https://exbridge.jp/outsourcing/">業務のAI自動化</a><a href="${publicBase}/vibe-oss.html">OSSのバイブコーディング・カスタマイズ</a><a href="https://exbridge.jp/ai-development.html?ref=kurage-oss">AI開発・活用支援</a><a href="https://exbridge.jp/nagoya-system-development.html?ref=kurage-oss">AI導入お試し</a><a href="https://exbridge.jp/ai-system/?ref=kurage-oss">AIでできること</a><a href="${PROTO}/?ref=kurage-oss">触れるデモ一覧</a><a href="https://kappstore.exbridge.jp/?ref=kurage-oss">買い切りの業務システム（Kurage App Store）</a><a href="https://exbridge.jp/system-development-cost.html?ref=kurage-oss-cost">業務システムの受託開発（名古屋・概算見積無料）</a><a href="https://exbridge.jp/">株式会社エクスブリッジ</a></div><p>Kurage Payload CMSは、業務OSSの選定、日本語導入、バイブコーディングによる自社向けカスタマイズを案内するカタログです。</p></div></footer>
</body></html>`
}
...
// 育っている入口(/oss/個票: zammad/codealmanac 等の技術者クエリが着地)から、
// 当社の完成品プロダクトへ内部リンクを流す(2026-09-06 GSC実測にもとづく)。
// 「OSSを探しているなら、同じ考え方で作った完成品もある」という関連文脈で置く。
function ownProductsStrip(): string {
  const P = [
    ['商圏分析', '住所→徒歩圏の人口・世帯・事業所', 'https://kurage.exbridge.jp/kshoken.php/?ref=oss-strip'],
    ['土砂災害ハザードマップ', '住所→警戒区域の内外を判定', 'https://kurage.exbridge.jp/khazard.php/?ref=oss-strip'],
    ['津波浸水想定マップ', '住所→何メートル浸かるか', 'https://kurage.exbridge.jp/ktsunami.php/?ref=oss-strip'],
    ['避難所マップ', '災害別に使える避難所まで徒歩何分', 'https://kurage.exbridge.jp/krefuge.php/?ref=oss-strip'],
  ]
  return `<section class="panel" id="own-products">
<h2>OSSを探しているなら、当社の完成品も</h2>
<p class="sub">同じ「行政・公共データを住所から使える形にする」考え方で、当社が作って公開しているプロダクトです。すべて無料で試せます。</p>
<div class="own-grid">
...
const categorySeo: Record<string, { title: string; desc: string; lead: string }> = {
  // 2026-09-06 実測: 安否確認 18,100/指数10・安否確認システム 12,100/指数17・BCP対策 8,100/指数14。
  ...
  //   正面は重いが在庫が280件あるので、掲載数そのものが差別化になる。
  groupware: {
    title: 'グループウェアのOSS一覧｜社内ポータル・ワークフローを自社サーバーで | Kurage',
    desc: 'グループウェア、社内ポータル、ワークフロー、スケジュール共有に使えるオープンソースの一覧です。サイボウズやdesknet\'s NEOのようなSaaSを月額で契約せず、自社サーバーで持つ選択肢をまとめています。',
    lead: 'グループウェア・社内ポータル・ワークフロー・スケジュール共有に使えるオープンソースをまとめています。月額のSaaSを契約せずに自社で持つ選択肢です。',
  },
...
function detailPage(project: Project, projects: Project[]): string {
  ...
  const body = `<main class="wrap"><nav class="crumb" aria-label="パンくず"><a href="${catalogBase}/">OSS一覧</a> / <a href="${catalogBase}/c/${attr(project.category)}/">${h(categoryLabel(project.category))}</a> / ${h(project.name)}</nav><section class="detail-hero"><span class="eyebrow">${h(copy.eyebrow)}</span><h1>${h(project.name)}</h1><p class="lead">${h(project.summary)}</p></section><div class="detail-grid"><div>${project.videoUrl ? `<section class="panel pv-panel"><h2>${h(project.name)}の紹介動画</h2><video src="${attr(project.videoUrl)}"${project.videoPoster ? ` poster="${attr(project.videoPoster)}"` : ''} controls playsinline preload="none" width="1920" height="1080"></video></section>` : ''}<section class="panel"><h2>${h(project.name)}とは</h2><p>${h(project.description)}</p><p><strong>要点:</strong> ${h(project.summary)}</p></section><section class="panel"><h2>主な用途</h2><ul class="checklist">${textItems(project.useCases).map((item) => `<li>${h(item)}</li>`).join('')}</ul></section>${kitPanel ? `<section class="panel vibe-guide kit">${kitPanel}</section>` : ''}${categoryPanel ? `<section class="panel vibe-guide kit">${categoryPanel}</section>` : ''}<section class="panel vibe-guide"><h2>${h(copy.guideTitle)}</h2><p>${copy.guideBody}</p><a class="btn btn-main" href="${attr(copy.ctaUrl)}">${h(copy.guideCtaLabel)}</a></section><section class="panel measured"><h2>${h(project.name)}の日本語対応（実測）</h2><p>${h(jaVerdict(project))}</p>${jaEvidence(project)}<p class="note">GitHubの公開ファイル一覧から、日本語ロケールに当たるファイルを数えた結果です。配布用にまとめたビルド成果物は除いています。調査日時点の内容で、翻訳の網羅率や品質までは示しません。</p></section>${demoPanel(project.slug, project.name, `oss-${project.slug}`)}
