...
function detailPage(project: Project, projects: Project[]): string {
  ...
  const body = `<main class="wrap"><nav class="crumb" aria-label="パンくず"><a href="${catalogBase}/">OSS一覧</a> / <a href="${catalogBase}/c/${attr(project.category)}/">${h(categoryLabel(project.category))}</a> / ${h(project.name)}</nav><section class="detail-hero"><span class="eyebrow">${h(copy.eyebrow)}</span><h1>${h(project.name)}</h1><p class="lead">${h(project.summary)}</p></section><div class="detail-grid"><div><section class="panel"><h2>${h(project.name)}とは</h2><p>${h(project.description)}</p><p><strong>要点:</strong> ${h(project.summary)}</p></section><section class="panel"><h2>主な用途</h2><ul class="checklist">${textItems(project.useCases).map((item) => `<li>${h(item)}</li>`).join('')}</ul></section><section class="panel vibe-guide"><h2>${h(copy.guideTitle)}</h2><p>${copy.guideBody}</p><a class="btn btn-main" href="${attr(copy.ctaUrl)}">${h(copy.guideCtaLabel)}</a></section><section class="panel measured"><h2>${h(project.name)}の日本語対応（実測）</h2><p>${h(jaVerdict(project))}</p>${jaEvidence(project)}<p class="note">GitHubの公開ファイル一覧から、日本語ロケールに当たるファイルを数えた結果です。配布用にまとめたビルド成果物は除いています。調査日時点の内容で、翻訳の網羅率や品質までは示しません。</p></section>
