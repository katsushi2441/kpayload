...
FACTS = {
    "bousai": "当社の道具: Kurage 土砂災害ハザードマップ（住所→警戒区域/特別警戒区域を判定）、津波浸水想定マップ（住所→浸水深と海抜）、避難所マップ（災害種別で使える避難所まで徒歩何分か）。いずれも名古屋市版デモが無料で触れ、オンプレミスで55,000円（税込）、政党・議員事務所が『事務所の名前』で公開できる。感震ブレーカーや耐震改修の助成は制度ナビ（名古屋市版）に収録。",
...
COMMON = "共通の事実: 当社（株式会社エクスブリッジ・名古屋）は架空政党『Kurage党』を実験場として住民サービスのページを先に動かし、全政党・全会派・無所属・政治団体・議員事務所に同じ条件で提供する。AI-IT顧問契約（月15時間・税別150,000円・名古屋市内限定）は政党・政治団体も対象で、キャンペーン期間中はKurage App Storeの商品代金が無料。名古屋市外はオンプレミス商品と導入キットを全国で使える。"
